...
def setup_workspace_assets(workspace_root: str | Path, config: "Config") -> None:
    ...
    # Create .sibyl/project structure
    sibyl_dir = root / ".sibyl" / "project"
    sibyl_dir.mkdir(parents=True, exist_ok=True)
    (sibyl_dir / "overlays").mkdir(exist_ok=True)
...
def _generate_claude_md(workspace_root: Path, config: "Config") -> str:
    ...
    return f"""# Sibyl Research System — Workspace Instructions
...
def update_gitignore(workspace_root: str | Path) -> None:
    ...
    from sibyl.orchestration.constants import RUNTIME_GITIGNORE_LINES
    root = Path(workspace_root)
