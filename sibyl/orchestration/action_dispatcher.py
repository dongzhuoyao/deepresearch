...
def _script_skill(action: Action) -> str:
    ...
    lines = [
        ...
        f"  - Run: sibyl cli-record . {action.stage} '<result_summary>' <score>",
    ]
    return "\n".join(lines)
...
def _script_skills_parallel(action: Action) -> str:
    ...
    lines.extend([
        ...
        f"  - Run: sibyl cli-record . {action.stage} '<combined_summary>' <score>",
    ])
    return "\n".join(lines)
...
def _script_team(action: Action) -> str:
    ...
    lines.extend([
        ...
        f"  - Run: sibyl cli-record . {action.stage} '<summary>' <score>",
    ])
    return "\n".join(lines)
...
def _script_bash(action: Action) -> str:
    ...
    lines = [
        ...
        "",
        f"Step 3: Record result",
        f"  - Run: sibyl cli-record . {action.stage} '<output_summary>' <score>",
    ]
    return "\n".join(lines)
...
def _script_gpu_poll(action: Action) -> str:
    lines = [
        ...
        "",
        f"Step 3: Record result",
        f"  - Run: sibyl cli-record . {action.stage} 'GPUs acquired' 0",
    ]
    return "\n".join(lines)
...
def _script_experiment_wait(action: Action) -> str:
    ...
    lines = [
        ...
        f"  - Run: sibyl cli-record . {action.stage} '<results_summary>' <score>",
    ]
    return "\n".join(lines)
